...
| 🎬 Media     | Netflix、Spotify、YouTube、Disney+、海外媒体合并为统一媒体分流组                      |
| 🫙 自定义    | 放自建节点，搭配上方分流规则使用                                                      |

其他分流规则，包括 哔哩哔哩(直连)、Khan、游戏平台等常用规则。

## 重要提醒

部分AI网站和APP优先使用了 `HTTP/3 (QUIC)`协议，导致即使分流和TUN 模式下都可能无法使用服务。

**此规则已内置于 `AiDomain.list` 中，无需手动添加：**
